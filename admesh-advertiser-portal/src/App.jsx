  ...
  useEffect(() => {
    if (token) {
      advertiserAPI
        ...
        .catch(() =>
          setMessage(
            "⚠️ NEURAL LINK SEVERED: Failed to fetch marketplace inventory. Ensure backend is online.",
          ),
        );
    }
  }, [token]);
  ...
  const handleDeployCampaign = async (zone) => {
    try {
      await advertiserAPI.createRule({
        ...
        html_payload:
          "<div style='padding:20px; background:#d946ef; color:white; text-align:center; border-radius:4px; font-family:monospace; border: 2px solid #fdf4ff; box-shadow: 0 0 15px #d946ef;'>⚡ NEON ADMESH INJECTED ⚡</div>",
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 relative">
        ...
        <div className="bg-black/40 backdrop-blur-xl p-10 rounded-2xl border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.15)] w-full max-w-md relative z-10">
          ...
          {message && (
            <p className="mt-6 text-fuchsia-400 font-mono text-center text-sm drop-shadow-[0_0_5px_rgba(217,70,239,0.8)]">
              {message}
            </p>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 pb-6 border-b border-cyan-500/20">
          <div>
            ...
            <p className="text-slate-400 font-mono text-sm mt-1">
              Select publisher nodes to inject payload.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {inventory.map((item) => (
            <div
              key={item.id}
              className="bg-slate-900/40 backdrop-blur-md border border-cyan-500/20 rounded-xl p-6 hover:border-cyan-400/60 transition-all duration-300 hover:shadow-[0_0_25px_rgba(6,182,212,0.15)] group flex flex-col"
            >
        ...
        {inventory.length === 0 && !message && (
          <div className="text-center py-20 border border-dashed border-slate-700 rounded-xl bg-slate-900/20 backdrop-blur-sm">
            <p className="text-slate-500 font-mono">
              SCANNING NETWORK FOR AVAILABLE NODES...
            </p>
          </div>
        )}
      </div>
