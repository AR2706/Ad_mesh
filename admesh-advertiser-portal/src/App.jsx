...
async def deliver_ad(zone: str, publisher_id: str = "guest", background_tasks: BackgroundTasks = None):
    """Fetches the active payload, with MongoDB fallback for true persistence."""
    ...
    if not cached_html:
        rule = await rule_collection.find_one(
            {"zone": zone, "is_active": True}, 
            sort=[("created_at", pymongo.DESCENDING)]
        )
        if rule:
            cached_html = rule.get("html_payload")
            source = "mongodb_restored"
            await redis_client.set(cache_key, cached_html)
    
    if background_tasks:
        background_tasks.add_task(log_event_to_redis, "impression", zone, publisher_id)
        
    if cached_html:
        return {"status": "success", "zone": zone, "htmlContent": cached_html, "source": source}
